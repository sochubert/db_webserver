...
router.post('/', function(req, res, next) {
    ...
    var sql1 = 'SELECT CUSTOMER_ID FROM CUSTOMER WHERE CUSTOMER_ID';

    var sql2 = 'SELECT RESERVATION_ID FROM RESRVATION';


    var sql = 'INSERT INTO review (RESERVATION_ID, STAR_POINT, CONTENT, DATE, TITLE) VALUES(?, ?, ?, ?, ?)';
    connection.query(sql, [reservation_id, rating, content, content_title], function (error, results) {
        if (error) {
            console.log(error);
        }
        console.log(results);
